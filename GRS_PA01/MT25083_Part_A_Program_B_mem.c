...
#include "MT25083_PART_C_workers.h"   // Your header file where mem() is declared
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // 3) mem         -> function the thread will run (from workers.h)
        // 4) NULL        -> argument passed to cpu()
        pthread_create(&thread[i], NULL, mem, NULL);
    }
